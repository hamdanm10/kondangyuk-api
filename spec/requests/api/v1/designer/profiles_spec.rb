...
RSpec.describe 'API V1 Designer Profile', type: :request do
  path '/api/v1/designer/profile' do
    get 'Get own profile' do
      tags        'Designer | Profiles'
      produces    'application/json'
      description 'Returns the authenticated user profile. Accessible by designer only.'
      security    [ cookieAuth: [] ]

      response '200', 'profile returned' do
        let(:user) { create(:user, :designer) }
      ...
      response '403', 'forbidden — non-designer role cannot access' do
    ...
    patch 'Update own account' do
      ...
      response '200', 'account updated' do
        let(:user) { create(:user, :designer, password: 'Password12345!') }
        let(:body) do
