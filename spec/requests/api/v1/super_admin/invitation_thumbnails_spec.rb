...
RSpec.describe 'API V1 Invitation Thumbnail', type: :request do
  ...
  path '/api/v1/super_admin/invitations/{invitation_id}/thumbnail' do
    ...
    get 'Show thumbnail' do
      ...
      description 'Returns the invitation thumbnail proxy URL (or null). Accessible by super_admin only.'
      security [ cookieAuth: [] ]
    ...
    put 'Upload thumbnail' do
      ...
      produces 'application/json'
      description 'Uploads/replaces the invitation thumbnail (image only). Accessible by super_admin only.'
      security [ cookieAuth: [] ]
    ...
    delete 'Remove thumbnail' do
      tags 'Super Admin | Thumbnails'
      produces 'application/json'
      description 'Removes (purges) the invitation thumbnail — hard delete. Accessible by super_admin only.'
      security [ cookieAuth: [] ]
