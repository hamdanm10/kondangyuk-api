...
RSpec.describe 'API V1 Invitation Document Media', type: :request do
  ...
  path '/api/v1/super_admin/invitations/{invitation_id}/document/media' do
    ...
    get 'List document media' do
      ...
      description 'Lists media attached to an invitation document (proxy URLs). Accessible by super_admin only.'
      security [ cookieAuth: [] ]
    ...
    post 'Upload document media' do
      ...
      description 'Uploads a media file (image/audio/video) to an invitation document and returns its ' \
                  'stable proxy URL. Accessible by super_admin only.'
      security [ cookieAuth: [] ]
  ...
  path '/api/v1/super_admin/invitations/{invitation_id}/document/media/{id}' do
    parameter name: :invitation_id, in: :path, type: :string, required: true
